...
fn lines_to_moves(input: &str) -> Result<Vec<Vec<Move>>, ParseIntError> {
    input
        .lines()
        .map(|line|
            line.split(',').map(|segment| -> Result<Move, ParseIntError> {
...
mod tests {
    ...
    #[test]
    fn test_run1() {
        assert_eq!(solver1(&lines_to_moves("R8,U5,L5,D3\nU7,R6,D4,L4").unwrap()), 6);
        assert_eq!(solver1(&lines_to_moves("R75,D30,R83,U83,L12,D49,R71,U7,L72\nU62,R66,U55,R34,D71,R55,D58,R83").unwrap()), 159);
    ...
    #[test]
    fn test_run2() {
        assert_eq!(solver2(&lines_to_moves("R8,U5,L5,D3\nU7,R6,D4,L4").unwrap()), 30);
        assert_eq!(solver2(&lines_to_moves("R75,D30,R83,U83,L12,D49,R71,U7,L72\nU62,R66,U55,R34,D71,R55,D58,R83").unwrap()), 610);
